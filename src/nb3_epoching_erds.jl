...
# ╔═╡ b61e33c6-f308-11ed-0638-bf9ce06b9884
begin
	using MAT
	using DataFrames
	using Printf
...
# ╔═╡ 9a6d2866-572a-4cc6-b3dc-f31105f44da4
md"""# Epoching + ERD/S"""

# ╔═╡ 866c8734-f288-40d8-9c5a-676528d5ea79
TableOfContents()

# ╔═╡ 9eba91cb-0d79-4c7f-b276-64fee9e9b2ec
cd("/home/geiger/Playground/2023-Decoding/src")

# ╔═╡ 74582627-bc4b-4d91-8b0c-fd58a168bb52
begin
	fs = 200
	subjects = [1,2,3,4,5]
...
# ╔═╡ 15f61bc6-a711-4c56-ad96-a5c0607fcf98
md"""## 0 Import data"""

# ╔═╡ 57cd60a1-0323-47b0-b92d-dc88bfd403ed
begin
	...
	for sub in subjects
	    ...
	    for run in runs
			data_run = matread(@sprintf("../data/derivatives/features/S%i/S%i_run%i.mat",sub,sub,run))
...
# ╔═╡ 188de115-06b8-4b0e-988a-e1268453b5bf
begin
	lines(dataset[1][2]["mu"][125,4:24])
	lines!(dataset[1][2]["beta"][125,4:24])
	current_figure()
end

# ╔═╡ a4af6063-1838-46e2-b143-d223253535b9
md"""## 1 Events"""

# ╔═╡ 49c90883-0635-46ce-af77-5c69f34223ae
function splitEvents(events)	
	new_cols = ["thumb","index","middle","ring","pinky","rest"]	
	for col in new_cols
    	insertcols!(events, col => zeros(size(events, 1)))
	end
	
	for i in 2:size(events.type,1)
		if events.type[i] != events.type[i-1]
			if events.type[i] == 1
				events.rest[i] = 1
			elseif events.type[i] == 2
				events.thumb[i] = 1
			elseif events.type[i] == 3
				events.index[i] = 1
			elseif events.type[i] == 4
				events.middle[i] = 1
			elseif events.type[i] == 5
				events.ring[i] = 1
			elseif events.type[i] == 6
				events.pinky[i] = 1
			end
		end
	end
...
# ╔═╡ 849408b4-af9a-4ed1-85d0-c50f23a1936b
events = Array{Array{DataFrame}}(undef,length(subjects));

# ╔═╡ 2c7d6892-1ee8-4fea-ae51-c118aaac4764
begin
for sub in subjects
	events[sub] = Array{DataFrame}(undef,length(runs))
	for run in runs
		events_basic = DataFrame(latency=dataset[sub][run]["times"][2:150:end], type=dataset[sub][run]["events"][2:150:end])
		# 150 because the data was downsampled from 600 Hz to 200 Hz (factor 3) and then segmented in non-overlapping 0.25s windows, which is equivalent to 50 samples. 3*50=150.
		
		eventsAll = splitEvents(events_basic)

		push!(dataset[sub][run],"eventsAll"=>eventsAll)
		push!(dataset[sub][run],"eventsThumb"=>eventsAll[eventsAll.thumb .> 0,[:latency,:thumb]])
		push!(dataset[sub][run],"eventsIndex"=>eventsAll[eventsAll.index .> 0,[:latency,:index]])
		push!(dataset[sub][run],"eventsMiddle"=>eventsAll[eventsAll.middle .> 0,[:latency,:middle]])
		push!(dataset[sub][run],"eventsRing"=>eventsAll[eventsAll.ring .> 0,[:latency,:ring]])
		push!(dataset[sub][run],"eventsPinky"=>eventsAll[eventsAll.pinky .> 0,[:latency,:pinky]])
		push!(dataset[sub][run],"eventsRest"=>eventsAll[eventsAll.rest .> 0,[:latency,:rest]])
	end
end
end

# ╔═╡ 8686565c-4991-4ad1-9cda-40c6632bff26
md"""## 2 Epoch"""

# ╔═╡ f68504a8-4dde-46f1-a936-2f12de1c916a
begin
...
for run in runs
	for finger in fingers
		for freqband in freqbands
			data_e_One,t = Unfold.epoch(data = dataset[sub][run][freqband],
						tbl = dataset[sub][run][@sprintf("events%s",finger)],
						τ = (-0.5,7),
						sfreq = 4)
	
			push!(dataset[sub][run],@sprintf("%s%s",freqband,finger)=>data_e_One)
			push!(dataset[sub][run],"t"=>t)
		end
	end
end
end
...
# ╔═╡ 736adbaa-6e3a-45ae-9845-cc6b36644d3f
dataset_e = deepcopy(dataset);
# dataset_e["mu/beta..."] structure: channels x t x epochs

# ╔═╡ de77a543-1524-4ce4-934b-53bda4de9e9f
md"""## 3 ERD/S"""

# ╔═╡ eb04b5fe-cc1d-42fe-961b-939c4ab2693a
begin
	t_sref = collect(-0.5:0.25:0)
	sref = findall(t .>= minimum(t_sref) .&& t .<= maximum(t_sref))

	t_thresh = collect(1:0.25:2)
	sthresh = findall(t .>= minimum(t_thresh) .&& t .<= maximum(t_thresh))
	thresh = 0.975
...
for finger in fingers
for freqband in freqbands
	data_e = dataset_e[sub][run][@sprintf("%s%s",freqband,finger)]
	erds = similar(dataset_e[sub][run][@sprintf("%s%s",freqband,finger)])
for channel in 1:size(data_e)[1]
for epoch in 1:size(data_e)[3]
	p_n = data_e[channel,:,epoch]
	p_k = data_e[channel,sref,epoch]
	erds[channel,:,epoch] = p_n .- sum(p_k) / length(sref)
end
end
	push!(dataset_e[sub][run],@sprintf("erds_%s%s",freqband,finger)=>erds)
	mean_erds = mean(mean(erds[:,sthresh, :], dims=2), dims=3)
	push!(dataset_e[sub][run],@sprintf("mean_%s%s",freqband,finger)=>mean_erds)
end
end
end
end
...
# ╔═╡ b8a9ee1f-003f-4ffc-a34e-9a63647310d4
dataset_erds = deepcopy(dataset_e);

# ╔═╡ 027becb9-35ed-4daa-a635-2b1b676f0feb
begin
	pooledERDS = Dict{String, Any}() # pool subjects, runs, times, and epochs for channels, fingers and freqband
	pattern = "mean"
	
for sub in subjects
for run in runs
	for (key, value) in dataset_erds[sub][run]
		# Check if the key matches the pattern
		if occursin(pattern, key)
			# Check if the key already exists
			if haskey(pooledERDS, key)
				# Concatenate the values
				pooledERDS[key] = hcat(pooledERDS[key], value)
			else
				# Add the key-value pair to the dictionary
				pooledERDS[key] = value
			end
		end
     end
end
...
# ╔═╡ 39d3ad7a-ec3c-4df5-84e0-a5f1389fccac
begin
	mean_ERDS = Dict{String, Any}()
	for (key, value) in pooledERDS
		#concat_ERDS = vec(pooledERDS[key])
		#mean_perFinger = mean(filter(!isnan,pooledERDS[key]))
		#push!(mean_ERDS,key=>mean_perFinger)

		mean_perChannel = mean(pooledERDS[key],dims=2)
		push!(mean_ERDS,key=>mean_perChannel)
	end
end

# ╔═╡ d8034b8f-b2a8-4e21-a4c8-e8be1c2394a6
begin
println("Number of electrodes above 2.5th percentile:")
for finger in fingers
for freqband in freqbands
	for (key, value) in mean_ERDS
		if occursin(@sprintf("mean_%s%s",freqband,finger), key)
			all_mean_ERDS = vec(filter(!isnan,mean_ERDS[@sprintf("mean_%s%s",freqband,finger)]))

			# Calculate the 2.5th percentile
			threshold = quantile(all_mean_ERDS, 0.975)

			# Find supra-threshold channels 
			channels = findall(x -> x > threshold, mean_ERDS[@sprintf("mean_%s%s",freqband,finger)])

			# Channel indices
			chIx = [index[1] for index in channels]

			# Print number of supra-threshold channels per finger and freqband
			println(@sprintf("%s%s :",freqband,finger),size(chIx,1))

			push!(mean_ERDS,@sprintf("channels_%s%s",freqband,finger)=>chIx)
		end
     end
end
end
...
# ╔═╡ 5628ac06-ad30-4b82-944f-97da3d067561
md"""## 4 Prepare data for classification"""

# ╔═╡ c78498d4-d3df-45fa-b99c-0889873c4ffe
dataset_svm = deepcopy(dataset_erds);

# ╔═╡ 0f01da6d-6c32-4073-8f25-f63ec62c83d1
begin
	t_svm = collect(0:0.25:5)
	ssvm = findall(t .>= minimum(t_svm) .&& t .<= maximum(t_svm))
######################################################################################
# NOTE: This was intended to use only data in the ROI (0 - 5 s)
# I forgot to do this here tho. So I just classify over every timestep, because why not (except that it takes somewhat longer).
######################################################################################
end

# ╔═╡ 5bbafd12-3698-4344-aea9-ded54c42e4d1
# Create feature matrices
begin
	remove = ["meanerds_muThumb","meanerds_muIndex","meanerds_muMiddle","meanerds_muRing","meanerds_muPinky","meanerds_betaThumb","meanerds_betaIndex","meanerds_betaMiddle","meanerds_betaRing","meanerds_betaPinky"]
	
	n_channels, n_times, n_epochs = size(dataset_svm[1][1]["erds_muThumb"])
	features = Array{Dict{String,Any}}(undef,size(subjects,1))
	
for sub in subjects
featuresSub = Dict{String,Any}()
for run in runs
# Remove unnecessary key-value pairs
for key in remove
	delete!(dataset_svm[sub][run],key)
end
# Concatenate epochs for each freqband-finger pair and for all runs of each subject
for (key, value) in dataset_svm[sub][run]
	if occursin("erds", key)
		reshaped = reshape(dataset_svm[sub][run][key],(n_channels,n_times * n_epochs))
		if haskey(featuresSub, key)
			featuresSub[key] = hcat(featuresSub[key], reshaped)
		else
			featuresSub[key] = reshaped
		end
	end
end
end
...
# ╔═╡ a339653b-e1ef-4c96-a0a8-fbb935f44db3
md"""## 5 Save data"""

# ╔═╡ ab6e5d7c-76f9-40e2-b8f0-cbb4f12cb0d5
begin
	for sub in subjects
		filename = @sprintf("../data/derivatives/erd/S%i_features.mat",sub)
		filepath = dirname(filename)
		mkpath(filepath)
		#matwrite(filename, features[sub])
	end
end
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "f851b18fd902931ef36a8f27ff6f38eaf47e9d87"
